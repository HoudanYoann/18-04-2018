<?php
        ...
        // On met en liaison notre connect.php pour relier notre Base de données à notre index.php
require_once 'connect.php';

$order = '';
    // Attention à ne pas oublier les parenthèses manquantes.
...
if(!empty($_POST)) {
    ...
    // Attention, c'est filter_var et non filter_variable
    ...
    // Attention à ne pas oublier le "s" de $errors
    if (count($errors) > 0) {
        ...
        $insertUser->bindValue(':firstname', $post['firstname']); // Ne pas oublier de finir par un ";" et c'est Firstname et non fistname
...
$queryUsers = $db->prepare('SELECT * FROM users' . $order);
        ...
        if ($queryUsers->execute()) {
            $users = $queryUsers -> fetchAll(); // Il y avait un "-" en trop
        }
...
// Error sera un tableau
$errors = array();

// On vérifie si notre $errors sera NULL ou non
